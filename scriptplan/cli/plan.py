...
def validate_tjp_file(tjp_path: str) -> Path:
    ...
    if path.suffix != '.tjp':
        logger.warning(f"File does not have .tjp extension: {tjp_path}")
...
@click.option(
    ...
    'output_csv',
    is_flag=True,
    help='Generate CSV output instead of JSON.'
)
@click.option(
    '-o', '--output',
    type=click.Path(),
...
def report(
    ...
    try:
        ...
        if not tjp_file or tjp_file == '-':
            ...
            if verbose:
                logger.debug(f"Created temporary file from stdin: {stdin_temp_file}")

        else:
            # Validate input file
            if verbose:
                logger.debug(f"Validating input file: {tjp_file}")
        ...
        if verbose:
            logger.debug(f"Input file SHA256: {file_hash}")
        ...
        if verbose:
            logger.debug(f"Reading report from: {primary_output}")
        ...
        if output_format == 'json':
            ...
            try:
                ...
                if verbose:
                    logger.debug(f"Replaced report_id with SHA256 hash: {file_hash}")
        ...
        if stdin_temp_file and stdin_temp_file.exists():
            ...
            if verbose:
                logger.debug(f"Cleaned up stdin temporary file: {stdin_temp_file}")
